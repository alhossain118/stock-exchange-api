...
router.put('/profile', User.authorize({admin: false}), (req,res) => {
  User.findByIdAndUpdate(req.user._id, {$set: req.body}, { new: true }, (err, editedProfile) => {
    res.status(err ? 400 : 200).send(err || editedProfile);
  });
})
